...
const createOrder = async (req, res) => {
  try {
    ...
    // Determine initial payment and order status based on method
    ...
    const getAddressValue = (field) => {
      switch (field) {
        ...
        case "lastName":
          // Try to extract lastName from various sources
          const lastName =
            address.lastName ||
            address.last_name ||
            address.name?.split(" ").slice(1).join(" ");

          // If lastName is still empty, use a fallback
          // This handles cases where only a single name is provided
          return lastName && lastName.trim() !== ""
            ? lastName
            : (address.name ? "." : ""); // Use "." as placeholder if name exists but has no last name
        case "zipcode":
          return (
            address.zipcode ||
            address.zipCode ||
