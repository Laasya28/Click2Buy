...
const addProduct = async (req, res) => {
  try {
    ...
    // Collect only the images that exist from file uploads
    const fileImages = [image1, image2, image3, image4].filter(
      (item) => item !== undefined
    );
    ...
    // Add manually provided image URLs
    if (req.body.imageUrls) {
      try {
        const externalUrls = JSON.parse(req.body.imageUrls);
